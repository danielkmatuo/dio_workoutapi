...
async def query(id: UUID4, db_session: DatabaseDependency):
    ...
    if not atleta:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail=f"Atleta com o id {id} não encontrado."
        )
...
async def query(id: UUID4, db_session: DatabaseDependency, atleta_update: AtletaUpdate = Body(...)):
    atleta: AtletaOut = (
        await db_session.execute(select(AtletaModel).filter_by(id=id))
    ).scalars().first()
    ...
    atleta_update_model = atleta_update.model_dump(exclude_unset=True)
    for key, value in atleta_update_model.items():
